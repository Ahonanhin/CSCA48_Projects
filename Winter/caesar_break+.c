...
void caesar_breaker(unsigned char crypto_message[MAX_STR_LEN], unsigned char password[MAX_STR_LEN], unsigned char dictionary[10][MAX_STR_LEN])
{
 ...
 //              there are many ways to approach this problem, some better than others. Think carefully!
    ...
    int count = 0, a = 0;
    unsigned char template[MAX_STR_LEN];
    for (int i = 0; i < MAX_STR_LEN; i++){
        template[i] = 0;
    }
    strcpy(template, crypto_message);

for (int i = 0; i <10; i++){
    unsigned char  *len = template;

        count = 0;

        caesar_decipher(template, dictionary[i]);
...
int main()
{
    ...
    // caesar_breaker(a_passage[0],password,dictionary);
    // printf("Password=%s, decoded message=%s",password,a_passage[0]);
